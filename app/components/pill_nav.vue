...
watch(
	() => props.items,
	(items) => {
		circleRefs.value = Array.from({ length: items.length }).fill(null)
		tlRefs.value = Array.from({ length: items.length }).fill(null)
		activeTweenRefs.value = Array.from({ length: items.length }).fill(null)
	},
	{ immediate: true },
)
...
function setCircleRef(el: HTMLSpanElement | null, index: number) {
	...
	<div class="top-[1em] left-0 md:left-auto z-[1000] absolute w-full md:w-auto">
		...
		>
			<component
				...
				ref="logoRef"
				:to="isRouterLink(items?.[0]?.href) ? items[0].href : undefined" :href="!isRouterLink(items?.[0]?.href) ? items?.[0]?.href || '#' : undefined" aria-label="Home"
				role="menuitem" class="inline-flex justify-center items-center p-2 rounded-full overflow-hidden" :style="{
					width: 'var(--nav-h)',
					height: 'var(--nav-h)',
			...
			<div
				ref="navItemsRef" class="hidden relative md:flex items-center ml-2 rounded-full" :style="{
					height: 'var(--nav-h)',
					background: 'var(--base, #000)',
				}"
			>
				<ul role="menubar" class="flex items-stretch m-0 p-[3px] h-full list-none" :style="{ gap: 'var(--pill-gap)' }">
					<li v-for="(item, i) in items" :key="item.href || `item-${i}`" class="flex h-full" role="none">
						...
						>
							<span
								:ref="el => setCircleRef(el as HTMLSpanElement, i)" class="block bottom-0 left-1/2 z-[1] absolute rounded-full pointer-events-none hover-circle" :style="{
									background: 'var(--base, #000)',
							...
							<span class="inline-block z-[2] relative leading-[1] label-stack">
								...
								<span
									class="inline-block top-0 left-0 z-[3] absolute pill-label-hover" :style="{
			...
			<button
				ref="hamburgerRef" aria-label="Toggle menu" :aria-expanded="isMobileMenuOpen" class="md:hidden relative flex flex-col justify-center items-center gap-1 p-0 border-0 rounded-full cursor-pointer"
				:style="{
					width: 'var(--nav-h)',
					height: 'var(--nav-h)',
					background: 'var(--base, #000)',
