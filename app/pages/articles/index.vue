	...
	<UContainer>
		<u-page>
			<u-page-body>
				...
				<ClientOnly>
					<u-content-search
						v-model:search-term="query" shortcut="meta_k" :files="files" :navigation="navigation"
						:fuse="{ resultLimit: 42 }"
					/>
				</ClientOnly>
				<u-empty
					v-if="(articles?.length ?? 0) <= 0" title="No interesting material yet"
					description="Give me some time to create some boring stuff." variant="naked"
					:actions="[{ label: 'Go back home', to: '/' }]"
				/>
				<u-blog-posts v-else>
					<u-blog-post
						v-for="article in articles" :title="article.title" :description="article.description"
